...
CONFIG = { "underlying_ticker": "NIFTY 50", "option_name": "NIFTY", "signal_day_of_week": 0, "strike_step": 50, "short_otm_pct": 0.03, "long_otm_pct": 0.06, "capital_pct": 0.35, "max_lots": 4, "hold_days": 5, }
...
def upgrade() -> None:
    ...
    strategy_id = conn.execute(sa.text("INSERT INTO strategies (code, name, is_active) VALUES (:code, :name, true) RETURNING id"), { "code": STRATEGY_CODE, "name": "NIFTY Iron Condor"}, ).scalar()

    strategy_version_id = conn.execute(sa.text("INSERT INTO strategy_versions (strategy_id, version, config, implementation_class, exit_evaluator_class, is_active) "
                                               "VALUES (:strategy_id, 1, :config, :implementation_class, NULL, true) RETURNING id"), {
                                                   "strategy_id": strategy_id,
                                                   "config": json.dumps(CONFIG),
                                                   "implementation_class": STRATEGY_CODE
                                               },
                                       ).scalar()
