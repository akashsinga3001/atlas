...
def run_trade_reconciliation(self) -> dict:
    """Cross-check pending equity trades against Kite order history and resolve any unconfirmed fills."""
    db = SessionLocal()
    try:
...
register(JobDefinition(name="TRADE_RECONCILIATION", display_name="Trade Reconciliation", description="Resolves pending equity trades against Kite order history", group="trading", task=run_trade_reconciliation))
