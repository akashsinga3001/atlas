...
class SignalRepository(BaseRepository[StrategySignal]):
    ...
    def get_signals(self, date_from: Optional[date] = None, date_to: Optional[date] = None) -> list[StrategySignal]:
        q = (self.db.query(StrategySignal).join(StrategyRun).options(joinedload(StrategySignal.strategy_run).joinedload(StrategyRun.strategy_version).joinedload(StrategyVersion.strategy)))
        if date_from:
