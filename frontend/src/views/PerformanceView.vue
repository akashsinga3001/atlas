<template>
  <div class="mx-auto flex max-w-[var(--content-max-width)] flex-col gap-5">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-xl font-semibold tracking-tight">Performance</h1>
        <p class="mt-1 text-sm text-[var(--color-text-tertiary)]">Portfolio-wide statistics and trends.</p>
      </div>
      <StaleBadge :last-updated-at="statsStore.resource.lastUpdatedAt" :has-error="statsStore.resource.status === 'error'" />
    </div>

    <BaseCard title="Portfolio stats" :icon="BarChart3">
      ...
      <div v-else-if="statsStore.resource.data" class="grid grid-cols-2 gap-6 sm:grid-cols-4 lg:grid-cols-6">
        <MetricTile label="Total P&L" :value="formatCurrency(statsStore.resource.data.total_pnl, { compact: true })" :tone="pnlTone(statsStore.resource.data.total_pnl)" />
        <MetricTile label="True return" :value="formatPercent(statsStore.resource.data.true_return_pct)" :tone="pnlTone(statsStore.resource.data.true_return_pct)" />
    ...
    <div class="grid grid-cols-1 gap-5 xl:grid-cols-2">
      <BaseCard title="Equity curve" :icon="LineChart">
        ...
        <PriceChart v-else :series="equitySeries" :height="240" />
      </BaseCard>

      <BaseCard title="NAV curve" :icon="TrendingUp">
        ...
        <PriceChart v-else :series="navSeries" :height="240" />
      </BaseCard>
    </div>
...
<script>
import { BarChart2, BarChart3, LineChart, PieChart, TrendingUp } from "@lucide/vue"
...
export default {
  ...
  components: { BaseCard, EmptyState, ErrorState, LoadingState, MetricTile, PriceChart, StaleBadge, CapitalAllocationCard, CashFlowCard, ReturnDistributionChart, SectorPerformanceList },
  data() {
    return { BarChart3, LineChart, TrendingUp, BarChart2, PieChart }
  },
  computed: {
    statsStore() {
    ...
    equitySeries() {
      return [{ name: "Cumulative P&L", color: "#37d6c8", data: (this.curveStore.equity.data ?? []).map((p) => ({ time: p.date, value: p.cumulative_pnl })) }]
    },
    navSeries() {
      return [{ name: "Account value", color: "#4ed08a", data: (this.curveStore.nav.data ?? []).map((p) => ({ time: p.date, value: p.total_value })) }]
    },
  },
  created() {
