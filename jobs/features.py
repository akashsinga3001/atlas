...
def daily_features_upsert(self, lookback_days: int = 90, backfill: bool = False) -> dict:
    """Compute and upsert OHLCV features for 1DAY, 1WEEK, 1MONTH candles."""
    logger.info('Starting daily features upsert task. lookback_days={} backfill={}', lookback_days, backfill)
    service = FeatureService()
    result = service.upsert_features(lookback_days=lookback_days, backfill=backfill)
...
def on_demand_features_upsert(self, reason: str = 'manual_run', lookback_days: int = 90, backfill: bool = False) -> dict:
    """Compute and upsert OHLCV features for manual execution."""
    logger.info('Starting on-demand features upsert task. reason={} lookback_days={} backfill={}', reason, lookback_days, backfill)
    service = FeatureService()
    result = service.upsert_features(lookback_days=lookback_days, backfill=backfill)
